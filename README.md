...
```bash
git add docs/path/to/new-article.md
...
**Sidebar 自動生成**：`sidebars.ts` 使用 `autogenerated` 模式，新增 `.md` 檔案後自動出現在 sidebar，不需手動更新任何設定。
...
2. 在 `docs/ai/` 下建立第一篇文章
3. 若目錄名稱自動格式化後不符合預期（如大小寫、含特殊符號），在目錄內新增 `_category_.json`：
   ```json
   { "label": "AI / ML" }
   ```
